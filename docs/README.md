# Project documentation

Cross-component architecture decisions live in [`adr/`](adr/). Each component
owns its detailed behavior, operating notes, sources, and decisions:

- [HTTP client and server](../http-server/README.md)
- [DNS server](../dns-server/README.md)
- [Reverse proxy](../reverse-proxy/README.md)
- [Load balancer](../load-balancer/README.md)
- [In-memory cache](../in-mem-cache/README.md)

The project uses a lean architecture decision record format described in
[`001-use-lean-nygard-inspired-adr-format.md`](adr/001-use-lean-nygard-inspired-adr-format.md).
The optional local backend registry is recorded in
[`003-use-local-leases-for-backend-discovery.md`](adr/003-use-local-leases-for-backend-discovery.md).
